...
//Helper function to upload buffer to Cloudinary with fallback for broken credentials
const uploadToCloudinary = (fileBuffer: Buffer): Promise<{ secure_url: string }> => {
    return new Promise((resolve, reject) => {
        const stream = cloudinary.uploader.upload_stream({ folder: "Savoria" }, (error, result) => {
            if (error) {
...
export const updateBookingStatus = async (req: AuthRequest, res: Response): Promise<void> => {
    try {
        ...
        //Verift booking belongs to the owner's restaurant
